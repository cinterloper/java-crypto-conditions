...
public class Ed25519FulfillmentSignatureSpi extends FulfillmentSignatureSpi {
	...
	protected void validateSignature(PrivateKey privateKey, byte[] signature) throws SignatureException {
		if(signature.length != Ed25519Fulfillment.SIGNATURE_LENGTH)
			throw new SignatureException("Signature size must be " +
					Ed25519Fulfillment.SIGNATURE_LENGTH + " bytes.");			
	}
	...
	protected Fulfillment getFulfillment(PrivateKey privateKey, byte[] signature) throws SignatureException {
		...
		if(a.length != Ed25519Fulfillment.PUBKEY_LENGTH)
			throw new SignatureException("Invalid key length. "
					+ "Expected " + Ed25519Fulfillment.PUBKEY_LENGTH + " bytes.");
	...
	protected PublicKey getInternalPublicKey(Fulfillment fulfillment) throws SignatureException {
		...
		try {
			EdDSAPublicKeySpec keySpec = new EdDSAPublicKeySpec(
					((Ed25519Fulfillment) fulfillment).getPublicKey(),
