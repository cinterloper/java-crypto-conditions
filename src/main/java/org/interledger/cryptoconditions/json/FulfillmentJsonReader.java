...
public class FulfillmentJsonReader {
	...
	public static Fulfillment nextFulfillment(JsonReader reader) throws IOException {
		...
		case PREFIX_SHA256:
			...
			prefixedFulfillment.setSubfulfillment(((JsonPrefixSha256FulfillmentPayload)payload).subfulfillment);
	 ...
	 * @param expectedName
	 * @throws IOException 
	 * @throws JsonParseException if the name that is read from the stream is 
	 * different to the supplied value
	 */
	public static void nextName(JsonReader reader, String expectedName) throws IOException {
		String name = reader.nextName();
		if(!name.equals(expectedName))
			throw new JsonParseException("Expected property name '" + expectedName + "' "
					+ "and got '" + name + "'.");
