...
public class Application {
	...
	public static void main(String[] args) 
		...
		KeyPair digestKeypair = new KeyPair(
				new Sha256DigestPublicKey(digest.digest(message)), 
				new Sha256DigestPrivateKey(message));
		
		KeyPairGenerator rsaKpg = KeyPairGenerator.getInstance("RSA");
		KeyPair rsaKeyPair = rsaKpg.generateKeyPair();
		AlgorithmParameterSpec pssParams = new PSSParameterSpec(
				"SHA-256", 
				"MGF1", 
		...
		testSignature("SHA256withRSA/PSS", rsaKeyPair, null, message, null);
...
//		testSignature("PrefixSha256Fulfillment", p, message);
	...
	private static void testSignature(String algo, KeyPair kp, Provider p, byte[] message, AlgorithmParameterSpec params)
		...
		if(params != null) {
			...
			if(params instanceof PSSParameterSpec) {
				...
				System.out.println("- mask digest: " + ((MGF1ParameterSpec)algoParams.getMGFParameters()).getDigestAlgorithm());
